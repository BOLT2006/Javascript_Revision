...
let arrayElements = ["green tea", "black tea", "chai", "oolong tea"];
let selectedTeas = [];
for(let i = 0; i < arrayElements.length; i++){
    if(arrayElements[i] == "chai"){
        break;
    }
    selectedTeas.push(arrayElements[i]);
}
...
let arrayElements = ["London", "New York", "Paris", "Berlin"];
let visitedCities = [];
for(let i = 0; i < arrayElements.length; i++){
    if(arrayElements[i] == "Paris"){
        continue;
    }
    visitedCities.push(arrayElements[i]);
}
